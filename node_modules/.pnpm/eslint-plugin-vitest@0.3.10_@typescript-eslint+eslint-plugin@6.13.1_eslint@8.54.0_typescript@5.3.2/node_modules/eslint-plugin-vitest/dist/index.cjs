...
const ValidVitestFnCallChains = /* @__PURE__ */ new Set([
  "beforeEach",
  "beforeAll",
  "afterEach",
  "afterAll",
...
function buildRegularExpression(pattern) {
  return new RegExp(
    `^${pattern.split(".").map((x) => {
      if (x === "**")
        return "[a-z\\d\\.]*";
...
function matchesAssertFunctionName(nodeName, patterns) {
  return patterns.some(
    (pattern) => buildRegularExpression(pattern).test(nodeName)
  );
}
    ...
    return {
      CallExpression(node) {
        ...
        if (isTypeOfVitestFnCall(node, context, ["test"])) {
          if (node.callee.type === utils.AST_NODE_TYPES.MemberExpression && (isSupportedAccessor(node.callee.property, "todo") || isSupportedAccessor(node.callee.property, "skip")))
            return;
          unchecked.push(node);
